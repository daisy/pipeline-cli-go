...
func loadDefault(cnf Config) error {
	//NP 2025/03/13 : first check if a config file is present in the current working directory
	cwd, err := os.Getwd()
	if err == nil {
	...
	//NP 2025/03/13 : check if the current executable is a symlink, and if so, get the real path
...
func (c Config) FromYaml(r io.Reader) error {
	...
	// > hardcoded default values [for webservice] are only used 
	// > if app_path is not set or the app can not be found
	// 
	// From my understanding, in order to implement requested behavior,
	// webservice config needs to be emptied and then rechecked 
	// after loading the yaml file and checking the app value
	c[HOST] = ""
	c[PORT] = 0
	c[PATH] = ""
...
func (c Config) Url() string {
	...
	if(path != "") { 
		// readd trailing slash at the end of the path
		path += "/"
	}
